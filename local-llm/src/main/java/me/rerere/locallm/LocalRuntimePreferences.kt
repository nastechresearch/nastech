...
class LocalRuntimePreferences(private val context: Context) {
     ...
     *  carries the rationale. The NastechApp crash sweep + the runtime's GPU->CPU
    ...
    /** Stamped by [NastechApp]'s ApplicationExitInfo sweep when the previous process
     ...
     * entries removed. Called once per app start from [NastechApp]'s SDK-aware sweep so
