 ...
 *  - **Native** — Nastech-format markdown with YAML frontmatter (`name:`, `description:`,
...
class SkillUrlImporter(
    ...
    private fun fetch(url: String): String {
        val req = Request.Builder()
            .url(url)
            .header("User-Agent", "nastech/skill-importer")
    ...
    private fun transcodeFromHermes(raw: String, sourceUrl: String, override: String?): String? {
        ...
        val body = buildString {
            ...
            appendLine("## Source")
            appendLine()
            appendLine("Imported from Hermes skill at $sourceUrl. Tool names were not transcoded — Hermes references its own tool surface, which doesn't always map 1:1 to Nastech's. Edit any tool references manually if the skill misbehaves.")
        }
    ...
    companion object {
        ...
        fun defaultClient(): OkHttpClient = OkHttpClient.Builder()
            ...
            .build()
            .also { io.github.nastechresearch.nastech.utils.NetworkChangeMonitor.register(it) }
    }
}
 ...
 *
 * openclaw uses some tool names that have Nastech equivalents but different spellings;
 * Hermes uses an entirely different set. We map those source names to the Nastech
...
object ToolNameTranscoder {
    private val Mappings: List<Pair<String, String>> = listOf(
        // openclaw → Nastech. Keys are case-sensitive; the lowercase common-English
        ...
        // Hermes-style → Nastech. These are snake_case identifiers that don't occur as
