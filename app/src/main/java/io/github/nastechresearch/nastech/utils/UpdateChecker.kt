...
class UpdateChecker(
                ...
                data = try {
                    val response = client.newCall(
                        Request.Builder()
                            ...
                            .addHeader(
                                "User-Agent",
                                "Nastech $currentVersionName #${BuildConfig.VERSION_CODE}"
                            )
                            .build()
                    ).await()
    ...
    /** Starts a user-visible download into app-owned storage. Android package installation stays user-confirmed. */
    fun downloadUpdate(context: Context, download: UpdateDownload): Boolean {
        return UpdateInstaller.enqueue(context, download) != null
    }
}
