...
import io.github.nastechresearch.nastech.service.AgentOverlay
...
import io.github.nastechresearch.nastech.data.ai.transformers.InputMessageTransformer
import io.github.nastechresearch.nastech.data.ai.transformers.MessageTransformer
import io.github.nastechresearch.nastech.data.ai.transformers.OutputMessageTransformer
import io.github.nastechresearch.nastech.data.files.FileFolders
import java.io.File
import java.io.IOException
import io.github.nastechresearch.nastech.data.ai.transformers.onGenerationFinish
...
private fun retryStatusText(
    ...
    io.github.nastechresearch.nastech.R.string.chat_page_retrying,
    retryNumber,
    maxRetries,
...
class GenerationHandler(
        ...
        for (stepIndex in 0 until maxSteps) {
            ...
            if (pendingTools.isEmpty()) {
                ...
                for (tool in tools) {
                    ...
                    val hardlineReason = io.github.nastechresearch.nastech.data.ai.tools
            ...
            toolsToProcess.forEach { tool ->
                when (tool.approvalState) {
                    ...
                    is ToolApprovalState.Pending -> {
                        ...
                        val resumeHardlineReason = io.github.nastechresearch.nastech.data.ai.tools
                            .HardlineCommandGuard.checkTool(tool.toolName, tool.input)
    ...
    /**
     * If the agent navigated away from Nastech during this turn (launch_app / open_url) and
     * the user is still on that destination, bring Nastech back to the foreground so the
    ...
    private fun handleAutoReturnAfterTurn() {
        ...
        // the user back to Nastech in that case defeats the purpose of the request.
        ...
        if (userSwitchedAway) {
            Handler(Looper.getMainLooper()).post {
                Toast.makeText(
                    ...
                    "Nastech: skipped auto-return because you switched apps. (Safety feature)",
                    Toast.LENGTH_LONG
                ).show()
            }
            return
