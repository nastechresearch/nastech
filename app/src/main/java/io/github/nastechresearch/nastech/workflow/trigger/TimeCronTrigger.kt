 ...
 * fire computed exactly via [io.github.nastechresearch.nastech.service.CronExpressionParser] (shared with
...
internal class TimeCronTriggerFamily(
    ...
    suspend fun onWorkerFired(workflowId: String) {
        ...
        val wf = lastSnapshot.firstOrNull { it.id == workflowId }
            ?: run {
                val loaded = io.github.nastechresearch.nastech.workflow.trigger.TimeCronWorkerHelper
    ...
    companion object {
        ...
        fun computeNextFireMs(spec: TriggerSpec.TimeCron, zone: ZoneId, nowMs: Long): Long {
            ...
            spec.cron?.trim()?.takeIf { it.isNotBlank() }?.let { cron ->
                io.github.nastechresearch.nastech.service.CronExpressionParser.parse(cron).getOrNull()?.let { parsed ->
                    io.github.nastechresearch.nastech.service.CronExpressionParser.nextExecution(parsed, now)
