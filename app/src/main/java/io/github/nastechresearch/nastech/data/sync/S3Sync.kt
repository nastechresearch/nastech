package io.github.nastechresearch.nastech.data.sync
...
import io.github.nastechresearch.nastech.data.db.ImportedDatabaseReconciler
import io.github.nastechresearch.nastech.data.files.FileFolders
import io.github.nastechresearch.nastech.data.files.SkillPaths
...
import io.github.nastechresearch.nastech.data.datastore.migration.SettingsJsonMigrator
import io.github.nastechresearch.nastech.data.sync.s3.S3Client
...
class S3Sync(
    ...
    suspend fun backupToS3(config: S3Config) = withContext(Dispatchers.IO) {
        ...
        val client = getS3Client(config)
        val key = "nastech_backups/${file.name}"
    ...
    suspend fun listBackupFiles(config: S3Config): List<S3BackupItem> = withContext(Dispatchers.IO) {
        ...
        val result = client.listObjects(
            prefix = "nastech_backups/",
            maxKeys = 1000
        ).getOrThrow()

        result.objects
            .filter { it.key.startsWith("nastech_backups/backup_") && it.key.endsWith(".zip") }
            .map { obj ->
                S3BackupItem(
                    key = obj.key,
    ...
    private suspend fun restoreFromBackupFile(backupFile: File, config: S3Config) = withContext(Dispatchers.IO) {
        ...
        // A backup exported from upstream Nastech lacks the fork-only tables; reconcile the
