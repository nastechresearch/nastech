 ...
 *
 * Bound by the OS once the user enables Nastech in Settings -> Notification access.
...
class RikkaNotificationListenerService : NotificationListenerService() {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification?) {
        ...
        runCatching {
            io.github.nastechresearch.nastech.workflow.trigger.NotificationTriggerDispatcher.onPosted(
    ...
    private suspend fun tryForwardToTelegram(entry: NotificationEntry) {
        ...
        if (entry.ongoing &&
            io.github.nastechresearch.nastech.data.ai.AgentTurnTracker.isFreshlyTouched(entry.packageName)) {
            return
        }
