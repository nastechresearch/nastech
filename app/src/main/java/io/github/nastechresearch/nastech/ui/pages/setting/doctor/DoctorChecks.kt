package io.github.nastechresearch.nastech.ui.pages.setting.doctor
...
import io.github.nastechresearch.nastech.data.ai.mcp.McpStatus
...
import io.github.nastechresearch.nastech.data.ai.tools.local.AgentWorkspace
import io.github.nastechresearch.nastech.data.ai.tools.local.NotificationListenerHandle
...
import io.github.nastechresearch.nastech.data.repository.ScheduledJobRunRepository
...
import io.github.nastechresearch.nastech.shizuku.ShizukuStatus
import io.github.nastechresearch.nastech.subagent.SubAgentModelResolver
...
import io.github.nastechresearch.nastech.browser.BrowserPreferences
import io.github.nastechresearch.nastech.browser.BrowserToolDefaults
...
class DoctorChecks(
        ...
        if (termuxInstalled) {
            ...
            add(
                DoctorCheck(
                    ...
                    detail = if (runCommandPerm) "Granted — Nastech can dispatch shell commands to Termux."
     ...
     * [io.github.nastechresearch.nastech.shizuku.ShizukuStatusMapper.compute]) rather than re-deriving the
     ...
     * pending, since [io.github.nastechresearch.nastech.NastechApp] runs the seed pass on every launch.
     */
     ...
     *    records ([io.github.nastechresearch.nastech.data.db.entity.GenMediaEntity]) whose backing file
    ...
    private fun diagnosticsChecks(enabled: Set<LocalToolOption>): List<DoctorCheck> = listOf(
        DoctorCheck(
            ...
            category = DoctorCategory.Diagnostics,
            label = "App build",
            detail = "Nastech-agent ${BuildConfig.VERSION_NAME} (${BuildConfig.VERSION_CODE}) — debug=${BuildConfig.DEBUG}",
            severity = Severity.INFO,
        ),
        DoctorCheck(
            id = "diag.android",
