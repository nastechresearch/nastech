...
import io.github.nastechresearch.nastech.ui.components.richtext.HighlightCodeVisualTransformation
...
private fun SearchServiceOptionsEditor(
    ...
    when (options) {
        ...
        is SearchServiceOptions.NastechOptions -> {
            NastechOptions(options) { onUpdateOptions(it) }
        }
...
@Composable
internal fun NastechOptions(
    options: SearchServiceOptions.NastechOptions,
    onUpdateOptions: (SearchServiceOptions.NastechOptions) -> Unit
) {
    FormItem(
        label = {
            Text(stringResource(R.string.search_detail_api_key))
