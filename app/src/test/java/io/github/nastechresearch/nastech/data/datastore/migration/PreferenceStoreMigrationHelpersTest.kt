package io.github.nastechresearch.nastech.data.datastore.migration
...
class PreferenceStoreMigrationHelpersTest {
    ...
    fun `mcp migration rewrites legacy fully-qualified sse type`() {
        val input = """
            [{"type":"io.github.nastechresearch.nastech.data.mcp.McpServerConfig.SseTransportServer","name":"a"}]
        """.trimIndent()
    ...
    fun `mcp migration rewrites legacy fully-qualified streamable http type`() {
        val input = """
            [{"type":"io.github.nastechresearch.nastech.data.mcp.McpServerConfig.StreamableHTTPServer","name":"b"}]
        """.trimIndent()
        val out = JsonInstant.parseToJsonElement(migrateMcpServersJson(input)).jsonArray
